import {generateTimeIntervals, includesWord, removeDuplicates} from "../utils/utils";
...
export const getEvents = async (selectedDate) => {
  ...
  try {
    const res = await fetch(`${process.env.REACT_APP_CALENDAR_EVENTS_URL}&q=listEvents&timeMin=${minDate.toISOString()}&timeMax=${maxDate.toISOString()}`)
...
export const addEvent = async (eventData) => {
  ...
  try {
    const res = await fetch(`${process.env.REACT_APP_CALENDAR_EVENTS_URL}&q=addEvent`, requestOptions)
    const data = await res.json()
    return data;
  }
  catch (error) {
